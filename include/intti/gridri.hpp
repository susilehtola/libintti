...
/// Grid-RI Coulomb matrix J_uv = sum_ls D_ls (uv|ls) via density-on-grid + DAGE:
/// rho = sum_uv D_uv chi_u chi_v, V = DAGE(rho), J_uv = <chi_u chi_v | V>. One
/// DAGE for the whole matrix. `tgrid` selects the kernel (coulomb for 1/r).
template <class Real>
std::vector<Real> grid_coulomb_build(const ShellBasis<Real> &basis, const Real *D,
                                     ...
                                     int nv = 24) {
  const int n = basis.nao, N = grid.N;
  const std::size_t N3 = static_cast<std::size_t>(N) * N * N;
  auto ao = ao_values_on_grid(basis, grid);
...
/// Grid-RI exchange matrix K_uv = sum_i (ui|vi) via co-densities g_ui = chi_u
/// phi_i, with occupied orbitals phi_i = sum_u Cocc[u*nocc+i] chi_u (density
/// D = Cocc Cocc^T). For each i: form the co-densities g_vi on the grid, DAGE
/// each to V_vi, and contract K_uv += <g_ui | V_vi>. nocc DAGEs per orbital.
template <class Real>

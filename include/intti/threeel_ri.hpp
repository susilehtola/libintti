...
#include "cdjk.hpp"     // ShellBasis-level two_step_cholesky (pivots + pair_shells)
#include "cholesky.hpp" // detail::syevd, CholeskyOptions
...
/// In-library auxiliary basis for the three-body RI folding, generated by the
/// two-step Cholesky decomposition of the ERI over the orbital pair space. The
/// CD selects the significant shell pairs (threshold opt.tau); each is
/// materialised as product-shell Gaussians -- shells l = 0..(l_i+l_j) at the
/// product centre P_ij with exponent alpha_i+alpha_j -- which span that pair's
/// product block exactly, so the collection spans rho_D to the CD threshold with
/// O(rank) ~ O(nao) auxiliaries (vs the full O(nao^2) pair space). No external
/// auxiliary basis needed; error controlled by opt.tau.
template <class Real>
ShellBasis<Real> cholesky_product_aux(const ShellBasis<Real> &orb, const TGrid<Real> &grid,
  ...
  auto seen = [&](Real ae, const Real Pc[3], int l) {
    for (const auto &s : aux)
      if (s.l == l && s.alpha == ae && s.center[0] == Pc[0] && s.center[1] == Pc[1] &&
          s.center[2] == Pc[2])
        return true;
    return false;
  };
  for (std::size_t p = 0; p < cb.pair_shells.size(); ++p) {
    if (!used[p]) continue;
    const auto &si = orb.shells[cb.pair_shells[p].first];
    const auto &sj = orb.shells[cb.pair_shells[p].second];
    const Real ae = si.alpha + sj.alpha;
    Real Pc[3];
    for (int d = 0; d < 3; ++d) Pc[d] = (si.alpha * si.center[d] + sj.alpha * sj.center[d]) / ae;
    for (int l = 0; l <= si.l + sj.l; ++l)
      if (!seen(ae, Pc, l)) aux.push_back(PrimitiveShell<Real>{ae, {Pc[0], Pc[1], Pc[2]}, l});
  }
  return make_basis(std::move(aux));
}

/// Three-body RI-folded energy using the in-library Cholesky auxiliary (no
/// external aux basis): builds cholesky_product_aux and calls
/// three_electron_energy_ri. `opt.tau` controls the auxiliary (CD) truncation.
template <class Real>
Real three_electron_energy_cd(const ShellBasis<Real> &orb, const std::vector<Real> &D,
                              const TGrid<Real> &grid, const CholeskyOptions<Real> &opt = {},
                              Real tau_ri = 1e-10) {
  return three_electron_energy_ri(orb, D, cholesky_product_aux(orb, grid, opt), grid, tau_ri);
}

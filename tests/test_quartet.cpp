...
using PrimitiveShell = intti::PrimitiveShell<double>;
...
TEST(Quartet, GoldenSSSS) {
  // (ss|ss) reference value from 40-digit arithmetic; the default Mobius
  // grid is converged to the working precision of the scalar type
  ...
  intti::eri_quartet(intti::make_pair(shell(0.8, kA, 0), shell(1.3, kB, 0)),
                     ...
                     &val);
  EXPECT_NEAR(val, 0.5625558912294552353350707, 1e-14);
}
...
TEST(Quartet, ShellsVsAnalytic) {
  ...
  for (const auto &lc : cases) {
    ...
    EXPECT_LT(err, 1e-13) << "shells " << lc[0] << lc[1] << lc[2] << lc[3];
  }
}

TEST(Quartet, LinLogShellsVsAnalytic) {
...
TEST(Quartet, TailCorrectionConvergenceLaw) {
  // LinLog grids: corrected error ~ 1/t_c^4, raw truncation error ~ 1/t_c^2
  ...
  const double ref = 0.5625558912294552353350707;
...
TEST(Quartet, ErfPlusErfcIsCoulomb) {
  ...
  intti::eri_quartet(bra, ket, intti::make_tgrid(intti::coulomb()), coul.data());
  intti::eri_quartet(bra, ket, intti::make_tgrid(intti::erf_rs(omega)), erf.data());
  intti::eri_quartet(bra, ket, intti::make_tgrid(intti::erfc_rs(omega)), erfc.data());
  double maxc = 0.0;
  for (int k = 0; k < nout; ++k)
    maxc = std::max(maxc, std::abs(coul[k]));
  for (int k = 0; k < nout; ++k)
    EXPECT_NEAR(erf[k] + erfc[k], coul[k], 1e-12 * maxc) << "component " << k;
}
...
TEST(Quartet, YukawaGridConvergence) {
  ...
  intti::eri_quartet(bra, ket, intti::make_tgrid(intti::yukawa(1.2)), coarse);
  intti::TGridSpec<double> spec;
  spec.n = 160;
  intti::eri_quartet(bra, ket, intti::make_tgrid(intti::yukawa(1.2), spec), dense);
  double maxd = 0.0;
  for (int k = 0; k < 9; ++k)
...
TEST(Quartet, CoincidentCenters) {
  // all centers equal: (ss|ss) = 2 pi^(5/2)/(pq sqrt(p+q)); the Mobius grid
  // is untruncated, so there is no tail error even here where the
  // four-orbital overlap is maximal
  ...
  EXPECT_NEAR(val, exact, 1e-13 * exact);
}

TEST(Quartet, ExtremeExponents) {
  // tight (1e4) x diffuse (1e-2) pairing on the default grid
  auto grid = intti::make_tgrid(intti::coulomb());
